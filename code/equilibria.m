function [s_Rx_ss, s_Ry_ss, s_R_ss] = equilibria(V_ss, R_ss, beta_ss, p)
    ...
    sol_R = fsolve(@(var_R) [tan(alpha_R_ss) - (var_R(2)/(1 + var_R(1))); ...
    ...
    omega_F = (V_ss*cos(beta_ss - delta_ss) + V_ss*p.l_F*sin(delta_ss)/R_ss)/((1 + s_Fx_ss)*p.R_w);
    omega_R = V_ss*cos(beta_ss)/((1 + s_Rx_ss)*p.R_w);
